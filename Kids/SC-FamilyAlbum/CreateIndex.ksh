...
##
##  to copy the required files to _FullSize:
##  for i in `grep \^DSC Section?? | awk -F\: '{ print $2 }' | awk '{ print $1 }'`
##  do
##    find /Volumes/Space2/ZenFolioRebuild/Events -name $i -print -exec cp {} _FullSize/ \;
##  done
##
...
PATH="~/bin:${PATH}:/Volumes/External300/GNU-old/bin:/Volumes/External300/DBProgs/RacePics"
...
<html>
<head>
<title>Ellen's Family</title>

<link rel=stylesheet href="CD.css" type="text/css">

<meta name="description" content="Images of the Ellen's family.  Photography by and Copyright Doug Greenwald, all rights reserved." />
<meta name="keywords" content="doug, greenwald, photography, ellen, family" />
<meta name="revision" content="${TODAY}" />

</head>
<body>

<p>The quick and dirty scorecard...</p>

<ul>

<li>Ellen
  <ul>
  <li>Crystal, married to Pablo
    <ul>
    <li>Sophia
    <li>Lucy
    </ul>  <!-- crystal's kids -->
  ...
  <li>Timothy
  <li>Pet - Henry, aka Muttley, Furface, Goof(ball)

  </ul>  <!-- ellen's kids -->

<li>Brenda 
  <ul>
  <li>Ellen's biological sister.  She is significantly older than the rest of the siblings and she found and adopted as many of them as she was able to in order to bring them all back together.
  </ul>  <!-- brenda -->

<li>Jenna - Ellen's biological sister.
  <ul>
  <li>Conner
  <li>Gabby
  </ul>  <!-- jenna  -->
...
</ul>  <!-- first level - ellen and peers  -->
...
<title>Ellen's Family - ${SectionTitle} - ${ImageFile}</title>
...
EOF
    ...
    ZFInfo=`~/bin/CSVQuery racepics "select zfgal, zfid from commerce where id = '${BaseName}'"`
